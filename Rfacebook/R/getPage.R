...
getPage <- function(page, token, n=25, since=NULL, until=NULL, feed=FALSE, reactions=FALSE, verbose=TRUE){
	...
	url <- paste0('https://graph.facebook.com/', page,
		'/posts?fields=from,message,created_time,type,permalink_url,story,comments.summary(true)',
		',likes.summary(true),shares')
	if (feed){
		url <- paste0('https://graph.facebook.com/', page,
		'/feed?fields=from,message,created_time,type,permalink_url,story,comments.summary(true)',
		',likes.summary(true),shares')
	}
	if (!is.null(until)){
